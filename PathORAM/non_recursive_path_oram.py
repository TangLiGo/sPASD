...
class PathOramClient:
    ...
    #
    # block dummy symbol used to pad a file to the maximum size which is (block_size - block_id_size)
    def __init__(self, level, Z=2, block_size=8192, block_id_size=32):
        #Z代表树上每个bucket能装多少个cipher
        self.Z = Z
        self.level = level
    ...
    def access(self, op, block_id, block_data, oram_server):
        ...
        #生成新的leaf映射
        self.position_map[block_id] = self.integer_to_position(randint(0, total_real_block))#用二进制串表示，这样对应位置的01代表的就是左子树还是右子树

     #   print("new pos",self.position_map[block_id])
        # read buckets along path block_position from server
        ...
        if op == 'write':
           ...
           # print(block_data,type(block_data),type(self.block_dummy_symbol),block_data)
            block_data = block_data + (self.block_size - len(block_data)) * self.block_dummy_symbol
            self.stash[block_id] = BlockPlaintext(block_id, block_data)#在stash中更新目标位置的数据
        #将刚刚read的路径上的数据都更新一遍，每个data block 更新的方式是比较自己映射的叶节点与目前路径的交点来
        for l in reversed(range(self.level + 1)):
           # print(block_position,l)
            intersect_block = self.find_intersection_block(block_position, l)
           # print(intersect_block)
          ...
          #  print("write",block_position,select_blocks_cipher,l)
