...
class DataOramClient:
    ...
    def read(self,block_id):
        ...
        block_data=self.server.read(block_id)
      ...
      #  print("main area:{}; holding area:{}".format(block_data, self.decrypt_block(self.server.read(holding_pos+self.N))))
        return self.server.read(holding_pos+self.dataORAM_mainarea_size),block_data
    def write(self, block_id, block_data):
        ...
        for refresh_id in refresh_range:
            self.server.write(refresh_id, self.read(refresh_id)[0])
    ...
    def generate_initialize_block(self):
        blocks = [self.generate_dummy_block() for i in
                  range(self.dataORAM_holdingarea_size + self.dataORAM_mainarea_size)]

        return blocks

    def generate_dummy_block(self):

        block_cipher = self.block_dummy_symbol * self.block_byte_size#BlockPlaintext(self.block_dummy_symbol * self.block_byte_size)
        return block_cipher
